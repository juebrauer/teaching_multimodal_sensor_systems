...
void draw_gaussian(int anchor_x, int anchor_y, double µ, double sigma, Scalar col)
{
  const double visualization_scale = 5000.0;

  int prev_draw_x = -1;
  int prev_draw_y = -1;
  for (double x = (int)µ - IMG_WIDTH; x <= (int)µ + IMG_WIDTH; x++)
  {
    double y = 1.0 / (sigma * sqrt(2*M_PI)) * exp(-0.5 * pow((x - µ) / sigma, 2.0));
...
int main()
{
  ...
  while (simulation_step<100000)
  {
    ...
    µ = µ + u + rnd_distribution_process_noise( generator );

    // 4. simulate sensor data
    double z = µ + rnd_distribution_measurement_noise(generator2);
    ...
    error_measurement = (error_measurement*N + abs(z - µ))                                     / (N+1.0);
    error_naive       = (error_naive      *N + abs(µ_naive_est - µ))                           / (N+1.0);
    error_kf          = (error_kf         *N + abs(myFilter.get_current_state_estimate() - µ)) / (N+1.0);
